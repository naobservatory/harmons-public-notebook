...
g_read_counts <- ggplot(read_counts_agg_long, aes(x=library, y=p_reads, color = read_group, group = library)) +
  geom_point(size = 5) +
  geom_line(color = 'black') +
  scale_y_log10(name = "Unique human-viral read fraction", labels = label_log(digits=2)) +
  theme_kit + 
  coord_flip()
g_read_counts
```
...
```{r}
#| label: plot-rhabdoviridae-histogram
#| fig-width: 10
#| fig-height: 2.5
...
```
```{r}
#| label: plot-rhabdoviridae-composition
#| fig-width: 10
#| fig-height: 7.5
#| warning: false
