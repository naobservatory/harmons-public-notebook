...
subtitle: "Pooled plasma from Iran"
...
library(ggbeeswarm)
library(latex2exp)
image_dir <- image_dir <- '/Users/harmonbhasin/work/securebio/lab_meeting/blauwkamp'
...
The two dominant viruses we see are Anellovirdae and Phycodnaviridae.
...
ra_dot
#ggsave(sprintf('%s/ra_viral_dot.jpg',data_dir), ra_dot, height = 8 ,width = 12)

```

Here is the same dot plot, with Anelloviridae removed so that we can see the other species better.
...
ra_dot <- ggplot(play_special, aes(x = ra_reads_hv, y=name)) +
  ...
  theme_light() + 
    ...
    panel.background = element_blank())
ra_dot
#ggsave(sprintf('%s/ra_viral_dot_wout_anello.jpg',data_dir), ra_dot, height = 8 ,width = 12)
