...
#samples_keep <- read_counts %>% filter(n_reads_hv > 1) %>% pull(sample)
samples_keep <- read_counts %>%  pull(sample)
...
library_with_total <- libraries %>% group_by(location) %>% mutate(total_samples = n_distinct(sample)) %>% ungroup()
...
plot_viral_family_composition <- function(taxid_chosen, threshold_major_species = 0.1) {
  ...
  species_counts_major <- species_counts %>%
    mutate(name_display = ifelse(name %in% species_major_tab$name, 
                                 name, "Other")) %>%
    group_by(sample, location, name_display) %>%
  ...
  species_counts_display <- species_counts_major %>%
    ...
    right_join(library_with_total, by = c('sample', 'location')) %>% 
...
plot_viral_family_histogram <- function(taxid_chosen) {
  ...
  family_data <- hv_family_counts %>%
    right_join(library_with_total, by = c('sample')) %>% 
    ungroup() %>%
    mutate(n_reads_hv = coalesce(n_reads_hv, 0)) %>%
    filter(taxid == taxid_chosen) %>%
    group_by(location) %>%
    summarize(n_reads_hv = sum(n_reads_hv), .groups="drop")
  ...
  g_histogram <- ggplot(family_data, aes(x = location, y = n_reads_hv)) +
...
```{r}
#| label: plot-hepadnaviridae-histogram
#| fig-width: 10
#| fig-height: 5
...
```
```{r}
#| label: plot-hepadnaviridae-composition
#| fig-width: 10
#| fig-height: 15
#| warning: false
...
```{r}
#| label: plot-orthoherpesviridae-histogram
#| fig-width: 10
#| fig-height: 5
...
```
```{r}
#| label: plot-orthoherpesviridae-composition
#| fig-width: 10
#| fig-height: 15
#| warning: false
...
```{r}
#| label: plot-retroviridae-histogram
#| fig-width: 10
#| fig-height: 5
...
```
```{r}
#| label: plot-retroviridae-composition
#| fig-width: 10
#| fig-height: 15
#| warning: false
...
```
```{r}
#| label: plot-adenoviridae-composition
#| fig-width: 10
#| fig-height: 15
#| warning: false
#| 
