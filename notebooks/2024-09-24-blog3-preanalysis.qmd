...
| Thijssen et al. (2023)       | Plasma          | Yes                   | DNA + RNA           | Iran        | 116M           | [link](https://doi.org/10.3390/v15071425)              |
| Mengyi et al. (2023)         | Plasma          | No                   | DNA                 | China       | 3.4B           | [link](https://doi.org/10.1016%2Fj.onehlt.2023.100602) |
| Thompson et al. (2023)       | Whole blood     | No (human focused study)                   | RNA                 | USA         | 27.7B          | [link](https://doi.org/10.1038/s41591-022-02107-4)     |
| O'Connell et al. (2023)      | Whole blood     | No (human focused study)                 | RNA                 | USA         | 3.1B           | [link](https://doi.org/10.1186/s12863-024-01223-z)     |
| Aydillo et al. (2022)        | Whole blood     | No (human focused study)                   | RNA                 | USA         | 1.7B           | [link](https://doi.org/10.1038/s41541-022-00583-w)     |
...
TODO: Cebria-Mendoza does viral enrichment. Briefly ... . More information can be found [here](https://www.nature.com/articles/s41598-021-86427-4).
...
TODO: Thijseen does viral enrichment. Briefly ... . More information can be found [here](https://www.nature.com/articles/srep16532)
...
*Full analysis can be found [here](https://data.securebio.org/harmons-public-notebook/notebooks/2024-09-12-thompson.html)*
...
*Full analysis can be found [here](https://data.securebio.org/harmons-public-notebook/notebooks/2024-09-05-oconnell.html)*
...
*Full analysis can be found [here](https://data.securebio.org/harmons-public-notebook/notebooks/2024-09-19-aydillo.html)*
...
library(pacman)
pacman::p_load(tidyverse, RColorBrewer, forcats, scales, ggbeeswarm, knitr, kableExtra, ggpubr)
...
formatted_datasets <- c("Cebria-Mendoza et al. (2021)", "Thijssen et al. (2023)", "Mengyi et al. (2023)", "Thompson et al. (2023)","O'Connell et al. (2023)", "Aydillo et al. (2022)")
#datasets <- c("thijssen2023", "mengyi2023",  "thompson2023","oconnell2023", "aydillo2022")
#formatted_datasets <- c("Thijssen et al. (2023)", "Mengyi et al. (2023)",  "Thompson et al. (2023)","O'Connell et al. (2023)", "Aydillo et al. (2022)")
...
#| warning: false
#| fig-cap: "**Table 2**: Kingdom composition. Top: Composition across datasets. Bottom: Composition across sample types."

sampletype_comp_summ <- dataset_comp %>%
  group_by(sample_type, classification) %>%
  summarize(p_reads = mean(p_reads), .groups = "drop") %>%
...
dataset_sampletype_summ <- rbind(dataset_comp_summ, sampletype_comp_summ) 

dataset_sampletype_summ %>% 
  select(sample_type, dataset, classifications) %>%
  kable() %>% 
  kable_paper("striped",full_width = F, position = "center", html_font = "Helvetica") %>% 
  pack_rows("Average", nrow(dataset_sampletype_summ)-1, nrow(dataset_sampletype_summ))
```

```{r}
#| label: kingdom-fraction-table-without-human-reads
#| fig-cap: "**Table 3**: Kingdom composition controlled for human reads. Top: Composition across datasets. Bottom: Composition across sample types."
#| warning: false
...
sampletype_nonhuman_comp_summ <- nonhuman_dataset_comp %>%
  group_by(sample_type, classification) %>%
  summarize(p_reads = mean(p_reads), .groups = "drop") %>%
  mutate(p_reads = sprintf("%0.5f %%", p_reads*100)) %>%
  pivot_wider(names_from = classification, values_from = p_reads) %>%
...
dataset_sampletype_nonhuman_summ <- rbind(nonhuman_dataset_comp_summ,sampletype_nonhuman_comp_summ) 

dataset_sampletype_nonhuman_summ %>% 
  select(sample_type, dataset, non_human_classifications) %>%
  kable() %>% 
  kable_paper("striped",full_width = F, position = "center", html_font = "Helvetica") %>%  
  pack_rows("Average", nrow(dataset_sampletype_nonhuman_summ)-1, nrow(dataset_sampletype_nonhuman_summ))
```
...
#| fig-height: 7.5
#| warning: false
#| fig-cap: "**Figure 2**: Relative abundance of human-infecting viruses across datasets (samples with 0 reads are filtered out)."
...
g_all_reads <- ggplot(read_counts, aes(x=dataset, y=p_reads, fill = sample_type)) +
  geom_boxplot(width=0.75, size = 1) +
...
```{r}
#| label: dataset-ra-hv
#| fig-cap: "**Table 4**: Relative abundance of human-infecting viruses in each dataset, with and without controlling for human reads."
...
summ_nonzero_counts <- read_counts %>%
  group_by(dataset, sample_type) %>%
  summarize(
    nonzero_fraction_5 = sprintf("%d/%d (%.1f%%)", 
                               sum(n_reads_hv > 5), 
                               n(), 
                               100 * mean(n_reads_hv > 5)),
    nonzero_fraction_0 = sprintf("%d/%d (%.1f%%)", 
                               sum(n_reads_hv > 0), 
                               n(), 
                               100 * mean(n_reads_hv > 0)),
    .groups = "drop"
  ) %>%
  rename(`Samples with > 5 HV reads` = nonzero_fraction_5, `Samples with HV reads` = nonzero_fraction_0)

sampletype_counts <- read_counts %>%
  group_by(sample_type) %>%
  summarize(mean_ra = mean(p_reads)) %>%
  mutate(mean_ra = sprintf("%.2e", mean_ra)) %>%
  rename(`RA` = mean_ra)

summ_nonhuman_sampletype_read_counts <- read_counts_nonhuman %>%
  group_by(sample_type) %>%
  summarize(mean_ra = mean(p_reads_not_human)) %>%
  mutate(mean_ra = sprintf("%.2e", mean_ra)) %>%
  rename(`RA (control for human reads)` = mean_ra)

sampletype_ra_summ <- left_join(sampletype_counts, summ_nonhuman_sampletype_read_counts, by="sample_type")

dataset_ra_summ <- read_counts %>% 
  group_by(dataset) %>% 
  summarize(mean_ra = mean(p_reads)) %>%
  mutate(mean_ra = sprintf("%.2e", mean_ra)) %>%
  rename(`RA` = mean_ra) %>%
  left_join(summ_nonhuman_read_counts, by="dataset") %>%
  left_join(summ_nonzero_counts, by=c("dataset")) %>%
  left_join(raw_read_counts %>% select(dataset, `Average number of reads`=rmean) %>% mutate(`Average number of reads` = sprintf("%.1f M", `Average number of reads` / 1e6)), by=c("dataset"))

dataset_ra_summ %>% kable() %>%
  kable_paper("striped",full_width = F, position = "center", html_font = "Helvetica")
...
#| fig-cap: "**Table 5**: Relative abundance of human-infecting viruses in each sample type, with and without controlling for human reads."
sampletype_ra_summ %>% kable() %>%
  kable_paper("striped",full_width = F, position = "center", html_font = "Helvetica")
...
g_hv_species <- ggplot(filtered_dataset_ra, aes(x = name, y = ra, color = sample_type, shape=dataset)) +
  geom_quasirandom(method = "quasirandom", size = 2) +
  scale_y_log10(limits = c(1e-08, 1), labels = label_log(digits = 3)) + 
...
#| fig-height: 15
#| warning: false
#| fig-cap: "**Figure 4**: Relative abundance of *selected* human-infecting viruses in each dataset. Top: without controlling for human reads. Bottom: controlling for human reads."
...
ggarrange(g_hv_species, g_hv_species_nonhuman, nrow = 2, common.legend = TRUE, legend = "bottom")
``` 
